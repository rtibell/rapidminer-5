...
/**
 * This function represents a tangens hyperbolic activation function by  calculating
 * (exp(weighted sum) - exp(- weighted sum)) / (exp(weighted sum) + exp(- weighted sum)) 
...
public class TanhFunction extends ActivationFunction {
    ...
    public double calculateValue(InnerNode node, Example example) {
        ...
        if (weightedSum < -45.0d) {
            ...
            double posexp = Math.exp(weightedSum);
            double nexexp = Math.exp(-1 * weightedSum);
            result = (posexp - nexexp) / (posexp + nexexp);
        }
        return result;
    }
